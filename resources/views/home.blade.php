                    ...
                    <div class="col-xs-12 ml-auto mr-auto ie-container-width-fix">
                        ...
                        <form id="flightForm" action="#" method="get" class="tm-search-form tm-section-pad-2">
                            <div class="form-row tm-search-form-row">
                                <div class="form-group tm-form-element tm-form-element-100" style="position: relative">
                                    <i class="fa fa-map-marker fa-2x tm-form-element-icon"></i>
                                    <input value="" required name="origin" type="text" class="form-control"
                                        id="inputCity1" placeholder="Enter Origin...">
                                    <div id="" class="dropdown-menu show w-100" style="display: none;"></div>
                                ...
                                <script>
                                    ...
                                    function setupCityAutocomplete(inputId) {
                                        ...
                                        const cityLists = (cities) => {
                                            let html = '';
                                            cities.forEach(item => {
                                                html += `
                    <div class="city-result-item clearfix p-2 border-bottom" data-label="${item.city}, ${item.country} – ${item.airport} (${item.value})" style="cursor: pointer;">
                        <div class="d-flex justify-content-between font-weight-bold">
                            <div>${item.city.toUpperCase()}, ${item.country.toUpperCase()}</div>
                            <div>${item.value}</div>
                        </div>
                        <div class="text-muted small mt-1">${item.airport}</div>
                    </div>
                `;
                                            });
                                        ...
                                        $input.on('input', function() {
                                            ...
                                            currentRequest = $.ajax({
                                                ...
                                                success: function(data) {
                                                    ...
                                                    //will be replaced by cityLists(data) [if approved]
                                                    let html = '';
                                                    data.forEach(item => {
                                                        html += `
                    <div class="city-result-item clearfix p-2" data-label="${item.city}, ${item.country} – ${item.airport} (${item.value})">
                                    ...
                                    `;
                                        ...
                                        $input.on('focus', function() {
                                            ...
                                            if (query === "") {
                                                cityLists(staticCities); // function to show the static list
                                            }
                                        });
